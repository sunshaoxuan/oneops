...
describe("authentication and RBAC user interface", () => {
  ...
  it("automatically starts Windows SSO with a recoverable local login", () => {
    ...
    expect(authPage).toContain("windowsSsoDestination(windowsSsoUrl)");
    expect(authPage).not.toContain("WINDOWS_SSO_SILENT_URL");
    expect(authPage).not.toContain('className="auth-sso-silent-frame"');
    expect(authPage).toContain("window.location.assign");
