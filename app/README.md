...
EnvPortal 数据使用一次性导入命令迁移。命令默认执行预演，只有传入 `--apply` 才写入数据库。导入器按文件哈希和脱敏行指纹去重，现有同名环境进入冲突报告，登录 ID、密码和数据库用户字段只计数。迁移同时读取 OneOps 已登记的机构别制品资料，将环境放入范围对应的分组，把 URL、DB 和 RDP 拆分为连接端点，并把有依据的制品版数登记为确认待候选。机构级制品列只保留为模块候选，只有与正式功能模块档案精确匹配后才建立模块关系。
