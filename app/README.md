...
用户注册、登录、会话、Windows SSO 自动建档和标准 RBAC 已接入工作中心网关。首次注册用户完成系统管理员引导，后续用户由系统管理员审核并分配系统范围或组织机构范围角色。Windows SSO 复用 EnvPortal 在 OHR0067 上的域认证结果，OneOps 独立维护用户、会话和角色。旧版 8998 缺少 AD 邮箱时，OneOps 可通过 `OPS_SSO_ALLOWED_WINDOWS_DOMAINS` 将可信 Windows 域账号映射到 `onehr.jp` 技术 UPN。
