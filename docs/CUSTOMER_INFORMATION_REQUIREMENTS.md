...
1. 顧客情報の Tab Bar 右端に常時表示する歯車 Icon の「Tab 設定」を配置する。
2. 利用者は七つの Tab の表示順を上移動及び下移動で変更できる。
3. 利用者は任意の Tab を非表示にできる。ただし、最低一つの Tab を表示状態として維持する。
4. 現在選択中の Tab を非表示にして保存した場合、保存後の先頭表示 Tab へ自動的に切り替える。
5. 「既定に戻す」は七つの Tab を既定順かつ全表示へ戻す。
6. 設定は現在の利用者物理 ID を含む Browser Storage Key へ保存し、同じ Browser で別利用者又は代理利用者へ切り替えた場合に設定を共有しない。
7. 設定は組織機関単位に分割せず、同じ利用者の全顧客へ適用する。
8. 新しい Tab が将来追加された場合、既存設定の末尾へ表示状態で自動追加する。
9. 保存操作で確定し、Cancel は変更を反映しない。
10. 現行契約は同じ Browser 内の個人設定とする。複数端末間の同期は Server 側個人設定 API を導入する別要件で扱う。
...
ロール編集の権限マトリクスでは、顧客情報 CAG 分析を「システム管理 > 顧客情報 CAG 分析」と表示し、実際の入口を明示する。顧客情報ページからは、`customer.knowledge.manage` を持つ利用者に限り、対象組織機関を維持したまま管理入口へ移動する軽量な案内を表示する。スキャン、再取込、再分析及び候補確認の操作本体はシステム管理の顧客情報 CAG 分析で提供する。
...
12. 候補の確認、反映及び却下にも `customer.knowledge.manage` を要求する。`customer.knowledge.scan` と `customer.knowledge.review` は現行の権限マトリクス及びロール保存対象へ表示せず、管理操作は `customer.knowledge.manage` へ統一する。
...
33. 顧客情報画面にナレッジスキャン領域が表示されず、管理権限を持つ利用者だけが案内ボタンからシステム管理の顧客情報 CAG 分析へ移動できる。管理画面では Code 順の組織機関選択、スキャン、再取込、再分析及び候補確認を実行できる。
