...
14. 自由会話は作成時の既定汎用 Model を使用する。クイックアシスタントは設定された開始 Model を使用する。
...
51. システム管理の AI設定配下に独立したクイックアシスタント画面を表示し、三言語名称、三言語説明、三言語入力開始例、カテゴリ、開始 Model、表示順、有効状態、継続指示を保存できる。
