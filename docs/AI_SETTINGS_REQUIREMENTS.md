...
OneOps 在“系统管理”中提供“AI 设置”，日文显示为“AI設定”。该功能统一维护 OpenAI 兼容模型接口与 Agent Gateway 接口。
...
2. AI 设置包含“模型 API”与“Agent Gateway”两个画面入口。
3. 通用任务与简单任务可以分别保存和测试模型设置。
4. 可以新增、编辑、删除和测试多个 Agent Gateway。
5. 设置画面占满系统管理内容工作区。
6. 每个设置卡片的测试与保存按钮位于卡片底部右侧，主操作位于最右侧。
7. Secret 刷新后完整回填并显示为密码字符。
8. SSE 代理保留事件格式并支持断线恢复。
9. 自动化测试、生产构建、数据库迁移、浏览器页面、控制台与截图验证全部通过。
