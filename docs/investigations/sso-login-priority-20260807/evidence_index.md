...
| 明示的なログアウト後にログイン画面へ戻る | `logout-login-page.png`、タスク内ローカル認証フィクスチャの DOM | 高 | フィクスチャ認証を使用 |
| ログアウト後に自動 SSO を再実行しない | ログアウト後 1.6 秒の URL `/`、SSO 自動要求 0 件 | 高 | フィクスチャ認証を使用 |
| 手動 SSO ボタンを維持する | ログアウト画面のボタン、フィクスチャの SSO 要求 1 件 | 高 | フィクスチャ認証を使用 |
