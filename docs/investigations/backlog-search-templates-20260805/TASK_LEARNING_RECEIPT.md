...
プロジェクトをテンプレートの境界とし、プロジェクト選択後に項目メタデータを取得する。テンプレートは項目 ID と項目名を保存し、検索時にテンプレート単位で実行する。全件を固定課題 DTO へ変換し、外部課題物理 ID で重複排除した後に件名順で並べ、ページングを行う。件名の初期値は昇順とする。
...
件名ソートでは、ページ変更イベントとソートイベントを分離し、空のソート状態を既定昇順へ戻す必要がある。Nginx reload の権限エラーが発生した場合は主サービス Health と upstream を確認し、静的 Portal の安全な配信経路と正式ローリング配信の状態を分けて記録する。
