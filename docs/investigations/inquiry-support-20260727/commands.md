...
D:\nginx\runtime\node\pnpm.cmd check
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/publish-portal.ps1 -SkipChecks -Reason inquiry-attachment-preview-20260727
Invoke-RestMethod -Uri http://127.0.0.1:8092/api/work-center/v1/health
```

画像、PDF、Word、Excel のプレビュー用上位ドロワー、その他形式の直接ダウンロード、ダウンロードへのフォールバック、ブラウザーコンソールを確認します。現行の詳細手順は `../inquiry-attachment-preview-20260727/commands.md` を参照してください。
