...
回答本文を受信せず失敗した会話末尾 Task には、失敗表示右端の小型文字 Button から同じ質問を再送信する操作を追加する。保存済みの質問、添付及び問合せ参照を使用し、同じ Session に新しい Task を一件作成する。過去の失敗 Task と回答本文を受信済みの Task には表示せず、自動再送信は行わない。
