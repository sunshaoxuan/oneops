...
| Model、推理レベル、速度を個別設定する | Migration 040、管理 API、階層 Dropdown | Gateway 226 件、Portal 190 件、DB 12 件 | 合格 |
| Model 選択時に既定値を取り込み個別変更できる | Form 状態更新、クイックアシスタント固有列 | TypeScript、Portal 回帰試験 | 合格 |
...
| master へ commit、push し origin/master と一致する | Git | 最終工程で確認 | 確認中 |
