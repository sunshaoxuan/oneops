...
| クイックアシスタントは推理レベルと速度を独立保存する | `app/db/migrations/040_expand_shortcut_starting_model_options.sql`、`app/gateway/ai-assistant-shortcut-database.mjs` | 高 | 有効 12 件を確認 |
| 管理画面は 3 項目の階層設定メニューを提供する | `app/apps/portal-shell/src/AiAssistantShortcutSettingsPage.tsx`、Portal 190 件、正式 Asset `index-QjDkbS_M.js` | 高 | 認証後の実画面は evidence_missing |
