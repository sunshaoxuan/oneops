...
| 項目 | 結果 | 証跡 |
|---|---|---|
| Portal 単体試験 | 合格 | 18 files、150 tests |
...
| HTTPS Portal | 合格 | 200、`index-BilYao3p.js` |
...
| 最下部 Icon Button | 合格 | 40px × 40px、文字なし、Button 下端 704px、Footer 下端 720px |
| Menu 内部 Scroll | 合格 | Menu client 454px、scroll 532px、overflow-y auto |
...
| Screenshot | 合格 | 展開時と折畳時の最下部 Icon Button を保存 |
...
最初の全体 Build 再実行時は並行する常時配信が `dist` を使用し、`icon-ai.svg` の Copy が `EBUSY` となった。配信終了後に全体 Check を先頭から再実行し、177 Gateway tests、14 Builder tests、150 Portal tests 及び Production Build が合格した。
