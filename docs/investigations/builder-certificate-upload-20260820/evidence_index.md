...
| 秘密鍵非永続化 | OneOps unit test と job metadata 検査 | 高 | Builder 32 件に含む |
...
| 正式配信 | Continuous Delivery Log と Runtime Worker | 高 | Port 対応は 17 時 47 分 52 秒成功、新 Worker は 17 時 47 分 39 秒起動 |
...
| 正式 Nginx 既定ポート | 最新実 `OneHrStandalone.zip` 内の四つの Nginx 設定 | 高 | HTTP 80、HTTPS 443、Dumi 8005 と 8006 |
| Redis 既定ポートと利用者 | `config.ini`、`redis.windows.conf`、`startup.cmd`、`install.ps1` | 高 | 6379、サービスと Backend が同じ `REDIS_PORT` を参照 |
| カスタムポート同期 | Builder 最終二重 ZIP 試験 | 高 | Nginx 18080、18443、18005、18006、Redis 16379 で検証 |
| ポート UI | 認証後 Browser、Console、Screenshot | 低 | `evidence_missing`。制御可能な認証済みタブなし |
