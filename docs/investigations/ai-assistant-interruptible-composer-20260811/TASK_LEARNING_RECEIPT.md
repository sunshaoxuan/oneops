...
- browser_coverage: 生成中 Draft、Paste、改行、Enter 抑止、添付 Lock、Stop、Stop 中 Lock、Cancelled、Session 隔離、保持 Draft 再送、自然完了、Reload、Console を確認した。停止処理中の Session 切替は Portal 競合 Test が証拠であり、正式 Browser 切替は Cancelled 終端後である。今回使用した Browser 操作 API では File Data を伴う Drag and Drop の直接注入を実行できなかったため、Disabled DOM、File Paste 及び Portal Test を代替証拠とした。
- release_lineage: `v0.18.18^{}` は Local と Remote で Application Commit `7231f36a30b3e3349c8f7238ca40f12fe111fd6c` に一致し、現行 `master` と `v0.18.19` の祖先である。
