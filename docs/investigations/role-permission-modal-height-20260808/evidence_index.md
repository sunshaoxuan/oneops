...
| Modal 本体と content は表示領域内に制限される | `app/apps/portal-shell/src/styles.css` の `.role-permission-modal`、`.ant-modal-content` | 高 | 認証済み画面の実測は未確認 |
...
| 認証済み Modal の寸法とスクリーンショット | Browser Use の IAB と Edge 状態 | 低 | SSO 遷移が URL ポリシーと Edge によりブロックされ `evidence_missing` |
