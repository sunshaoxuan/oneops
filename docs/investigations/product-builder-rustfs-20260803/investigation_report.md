...
4. 2026-08-04 時点の公式 Download Center `dl.rustfs.com/rustfs/` は `rustfs-windows-x86_64-v<version>.zip` を公式 CDN から提供する。バージョン固定 ZIP と `latest` ZIP が同時に存在する。
...
4. 構築器は公式 Download Center から版数を取得し、公式 CDN の Windows x86_64 ZIP を取得する。取得後は `rustfs/` ルートへ正規化し、専用 `start.bat` とバージョンメタデータを追加する。
...
- [RustFS Download Center](https://dl.rustfs.com/rustfs/)
