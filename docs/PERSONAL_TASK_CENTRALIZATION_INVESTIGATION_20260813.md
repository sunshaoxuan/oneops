...
外部終了状態の共通判定を Connector に追加し、Repository は終了案件を新規候補及び通知として登録しない契約に変更した。既存の `PENDING` 候補は Migration 054 で `STALE` へ移し、対応する通知を削除した。利用者が既に採用した `ADOPTED` タスク 1 件は、利用者の明示操作を保持するため自動削除していない。

実 Migration 後は `PENDING=0`、通知 0 件となった。実外部同期で Backlog 1 件と問合せ 0 件を取得し、新規候補と通知はどちらも 0 件であった。ブラウザは候補 Tab の数値 Badge を表示せず、「確認待ちの候補はありません」を表示した。
