...
ユーザー編集 Dialog は「基本情報」「外部システム対応」「ロールと権限」「所属と職務」の 4 Tab に分割する。編集対象ユーザーの要約は Tab の上に固定表示し、Tab 切替中も未保存の入力値を保持する。Dialog Footer の保存及びキャンセルは全 Tab に共通とする。画面全体に長大な Scroll を発生させず、必要な場合は選択中 Tab の内容領域だけを Scroll する。
