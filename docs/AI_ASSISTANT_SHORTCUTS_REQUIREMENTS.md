...
各クイックアシスタントは開始表示及び監査起点となる Model と推理強度を持つ。Model 選択時は Model 設定の推理強度を既定値として取り込み、管理者はクイックアシスタント単位で変更できる。速度は Model の説明属性とする。Task 実行時は AI助手全体の軽量、複雑、同一 Task 再実行 Routing Policy を適用する。
...
14. Session 作成後に Model 設定又はクイックアシスタント設定を変更しても、既存 Session の開始表示用 Model、推理強度、Model 由来の速度及び継続指示スナップショットが変化しない。Task 実行 Model は AI助手全体の Routing Policy で決定する。
