...
各クイックアシスタントは開始 Model と推理強度を持つ。Model 選択時は Model 設定の推理強度を既定値として取り込み、管理者はクイックアシスタント単位で変更できる。速度は Model の説明属性とする。Session 作成時に Model、推理強度及び Model の速度を固定し、後続発言と同一入力の再実行でも同じ実行契約を使用する。
