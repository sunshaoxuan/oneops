...
11. クイックアシスタント入口は通常時に完全な円形輪郭を持つ静止 Icon として表示する。同じ操作領域の「新しい話題」ボタンへマウスを置いた時又はキーボードフォーカスを置いた時だけ、円形輪郭を分割軌道へ切り替えて Icon と共に動かす。
12. OS 又は Browser の動きを減らす設定が有効な場合は、操作中も完全な円形輪郭と Icon を静止表示する。
