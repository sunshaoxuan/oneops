...
各クイックアシスタントは Session の実行 Model と推理強度を持つ。Model 選択時は Model 設定の推理強度を既定値として取り込み、管理者はクイックアシスタント単位で変更できる。速度は Model の説明属性とする。選択した Model と推理強度は作成した Session の全 Task で固定使用する。
...
7. 翻訳機能は目標言語だけを出力し、固有名詞又は翻訳不能な用語を除いて原文言語の助詞、語尾及び機能語が残っていないことを送信前に確認する。
8. 日中相互翻訳は各 Turn の現在入力から翻訳方向を判定する。過去の翻訳方向、原文又は訳文を現在入力へ混在させず、日本語入力は中国語、中国語入力は日本語として独立して生成する。

## 4. 利用者画面

1. 完全画面の会話履歴上部で、「新しい話題」と右端の二重矢印を一体型の分割 Button として表示する。
2. AIアシスタント Header には新規話題操作とクイックアシスタント入口を表示しない。
...
11. AIアシスタント Header には「新しい話題」とクイックアシスタントの入口を表示しない。履歴ガイドの「新しい話題」を主 Segment、右端の二重矢印を Menu Segment とする一体型分割 Button を表示する。
12. 通常時は分割 Button と二重矢印を完全に静止させる。分割 Button 全体へ Hover 又は Keyboard Focus がある間だけ二重矢印を発光させ、二重矢印の Menu Segment へ Hover、Keyboard 実行又は Click がある時だけカテゴリ別 Menu を表示する。Reduced Motion では発光 Animation を停止して静的強調だけを表示する。
13. カテゴリ別 Menu は分割 Button の Container を位置基準とし、Page Scroll 又は会話履歴の長さに影響されず二重矢印の直下へ表示する。
14. 第2階層 Menu は分割 Button Row を位置基準とし、第1階層 Menu の項目上端と揃える。821px 以上の完全画面では右側、561px 以上820px 以下では左側へ表示する。560px 以下及び浮動 Window では第1階層上へ重ね、Window の `overflow` と Viewport で裁切されない位置へ表示する。名称、説明、Model、推理強度及び速度を視認できること。
15. 第2階層 Menu の説明文は本文領域の全幅を使用し、日本語の禁則処理と末行最適化を適用する。句点又は一文字だけが次行へ孤立する配置を避け、カード間の行高と情報密度を揃える。購読 Icon は見出し行だけに配置し、説明文の有効幅を狭めない。

## 5. 継続指示

1. Session 作成時にクイックアシスタント物理 ID と継続指示のスナップショットを保存する。
2. 同じ Session の全発言で保存済みスナップショットを Responses API の `instructions` へ挿入する。
3. ブラウザーから継続指示を送信しない。
...
10. `sort_order`: カテゴリ内の表示順
11. `enabled`: 有効状態。開始 Model と推理強度が設定されている場合だけ有効化できる。
12. `created_by_user_id`、`updated_by_user_id`: 利用者物理 ID への外部キー
13. `created_at`、`updated_at`: 監査時刻
...
5. 管理者はカテゴリ、三言語名称、三言語説明、三言語入力開始例、開始 Model、開始時の推理強度、継続指示、表示順、有効状態を定義する。
...
9. 開始設定は Model と推理強度の 2 項目を持つ階層メニューで編集し、各項目の第 2 階層へ選択肢と現在値を表示する。
10. Model を変更した時は選択した Model 設定の推理強度を取り込み、その後の推理強度変更は選択中のクイックアシスタントだけへ適用する。
11. 階層メニューの下部と操作ボタンには現在の Model と推理強度の要約を表示する。速度は一覧の Model 情報として表示し、編集項目にしない。
...
2. 新しい話題と右端の二重矢印が一体型の分割 Button として表示され、通常時は完全に静止する。
...
14. Session 作成後に Model 設定又はクイックアシスタント設定を変更しても、既存 Session の実行 Model、推理強度、Model 由来の速度及び継続指示スナップショットが変化しない。全 Task は Session の保存済み Model と推理強度を使用する。
15. 管理画面で Model と推理強度を個別に展開して選択でき、現在値に選択済み表示が付く。
16. Model 選択時に既定の推理強度が反映され、その後に推理強度を個別変更して保存できる。速度は Model 由来の表示情報となる。
17. カテゴリ別 Menu が二重矢印の直下で視認でき、Page Scroll と会話履歴の長さによって Viewport 外へ移動しない。
18. 全4 Category の第2階層 Menu が完全画面、560px 以下及び浮動 Window の表示領域内で表示され、各クイックアシスタントの名称、説明、開始 Model、推理強度及び速度を選択前に確認できる。
