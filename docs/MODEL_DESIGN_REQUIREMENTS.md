...
4. API Key：由管理员输入，在后端加密保存。系统管理员读取设置时，后端解密并回填完整值；前端使用密码输入框显示，不提供明文显示切换。

每条模型设置记录拥有独立、稳定的物理 ID。提供方名称不承担物理关联键职责。

已经保存 API Key 时，编辑画面回填完整 API Key 与“已配置”状态。输入框类型固定为密码，画面只显示密码字符；保存或测试时提交完整值，输入新值会替换旧值。
...
1. API Key 只通过需要系统管理员权限的 HTTPS 设置接口读取和提交，页面使用关闭明文切换的密码输入框显示。
...
1. `GET /api/work-center/v1/model-settings`：读取配置、API Key 已配置状态和供密码输入框回填的完整 API Key。
...
4. 提供方只有 `OpenAI`。
5. API Key 使用密码输入框，刷新后回填完整值并显示为密码字符，不提供明文显示切换。
